...
# Modified to include skipped preprocessor ranges functions.
...
class TranslationUnit(ClangObject):
    ...
    def get_skipped_ranges(self, filename):
        """Return an iterable (and indexable) object containing skipped preprocessor regions."""
        ranges = conf.lib.clang_getSkippedRanges(self, self.get_file(filename))
        return SourceRangeList(ranges)
...
class SourceRangeListStruct(Structure):
    _fields_ = [
        ("count", c_uint),
        ("ranges", POINTER(SourceRange))]
...
class SourceRangeList(ClangObject):
    def __init__(self, obj):
        assert isinstance(obj, POINTER(SourceRangeListStruct))
        self.obj = self._as_parameter_ = obj

    def __del__(self):
        conf.lib.clang_disposeSourceRangeList(self)

    def __len__(self):
        return len(self.obj.contents)

    def __getitem__(self, key):
        return self.obj.contents.__getitem__(key)
